...
class ConversationEngine:
    ...
    async def generate_response(
        ...
        if msg_lower in ["ابدأ", "ابدا", "start", "مهتم"]:
            return (
                ...
                f"جاهزين نرسلك الخطة التسويقية ومراجعة الحساب الحصرية. "
                f"ممكن تشاركنا رقم هاتفك أو بريدك الإلكتروني لإرسال الملف والمتابعة معك فوراً؟"
            ), False
        ...
        if any(w in msg_lower for w in ["سعر", "اسعار", "أسعار", "بكم", "بكام", "كام", "تكلفة", "اشتراك", "اشتراكات", "باقة", "باقات", "عروض", "cost", "price"]):
            return (
                f"أهلاً بك يا {lead_name}! بنوفر باقات مرنة ومتنوعة تناسب أهداف كل نشاط تجاري، "
                f"سواء إدارة إعلانات ممولة، أو صناعة ونشر ريلز احترافية، أو أتمتة الردود والمبيعات بالذكاء الاصطناعي. "
                f"لتحديد الباقة الأنسب لنشاطك وإرسال العرض المخصص، ما هو رقم الهاتف أو الواتساب الأنسب للتواصل معك؟"
            ), False

        # Trigger 3: Services inquiry
        if any(w in msg_lower for w in ["خدمات", "ايش تقدمون", "ماذا تقدمون", "خدمتكم", "services", "بتعملوا ايه"]):
            return (
                f"أهلاً بك {lead_name}! نحن في إبدأ ماركتينج بقيادة كريم عبد الواحد متخصصون في:\n"
                f"1. إدارة الحملات الإعلانية الممولة الموجهة للبيع المباشر.\n"
                f"2. صناعة سيناريوهات ومحتوى الريلز الفيروسي وتنمية الحسابات.\n"
                f"3. أنظمة الذكاء الاصطناعي للرد التلقائي وإغلاق الصفقات 24/7.\n"
                f"ما هو الهدف الأهم لحسابك حالياً؟"
            ), False

        # Default friendly response adhering to brand tone & inviting consultation
        return (
            f"مرحباً بك {lead_name}! يسعدنا تواصلك معنا دائماً في إبدأ ماركتينج. "
            f"كيف يمكننا مساعدتك اليوم في تنمية نشاطك التجاري ومضاعفة مبيعاتك؟"
        ), False
    ...
    async def _call_gemini_api(self, lead_data: Dict[str, Any], message: str, history: list) -> Optional[str]:
        ...
        system_prompt = (
            "أنت المساعد الذكي الرسمي والمسؤول عن إدارة المحادثات وإغلاق المبيعات لحسابات كريم عبد الواحد / إبدأ ماركتينج.\n"
            "مهمتك الرئيسية:\n"
