...
??????? ?????? ??? ?????? � ?? ???? ???? ??????? ??? ???????? **??????** ?? ????? ???????? (Fernet ???? ?? SECRET_KEY ??? `src/core/crypto.py`). **???? ???? ??????** � `user_entitlements` ?? ?????? ?????? (????? Entitlement ? Capability).
...
| `account_id` | VARCHAR(120) | ???? ?????? ?????? (page_id / ig id / threads id) |
| `account_name` | VARCHAR(160) | ??? ????? (?@username / ??? ??????) |
...
| `connected_at` / `created_at` / `updated_at` | TIMESTAMPTZ | ????? SOP-03 + trigger |

- UNIQUE(user_id, platform, account_id) � ????? user_id ?(user_id, platform, status) � RLS ???? ?????? (service_role ???? anon ?????).
- **Upsell ??????**: ??? ?????? ????? IG ????? ??????? ? metadata.linked_ig_username ? ????? upsell ?????? ?? ??? wizard ??? ???? entitlement:instagram.
- ??????: `src/modules/connections/service.py` (ConnectionService + assert_entitled fail-closed) � ????????: `/api/connections/*` (overview / authorize / callback / disconnect).
